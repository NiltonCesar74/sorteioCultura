<?php
  ...
  $sqlNum = "SELECT numeros from tb_grupo_folclorico where sorteado != 1 order by RAND() limit 1";
  $nroRes = $con->query($sqlNum);
  $nro = $nroRes->fetch(PDO::FETCH_OBJ);

  $sql = "UPDATE tb_grupo_folclorico SET `sorteado` =  1, data = NOW() where `numeros` = ? ";
